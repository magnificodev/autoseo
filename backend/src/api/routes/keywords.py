...
def create_keyword(
    body: KeywordIn, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    # Verify site exists
...
def update_keyword(
    ...
    return KeywordOut(
        ...
        if hasattr(keyword, "updated_at") and keyword.updated_at
        else keyword.created_at.isoformat()
...
def delete_keyword(
    keyword_id: int, db: Session = Depends(get_db), user=Depends(get_current_user)
):
    keyword = db.get(Keyword, keyword_id)
    if not keyword:
        raise HTTPException(status_code=404, detail="Keyword not found")
