...
export default function ContentPage() {
  ...
  const [form, setForm] = useState<Content>({ site_id: 1, title: '', body: '', status: 'pending' });
  ...
  return (
    <main style={{ padding: 16, fontFamily: 'sans-serif' }}>
      ...
      <form onSubmit={createContent} style={{ display: 'grid', gap: 8, maxWidth: 600 }}>
        <input type="number" placeholder="Site ID" value={form.site_id} onChange={(e) => setForm({ ...form, site_id: Number(e.target.value) })} />
      ...
      <ul>
        {items.map((c, i) => (
          <li key={i}>[{c.site_id}] {c.title} – {c.status || 'pending'}</li>
        ))}
      </ul>
    </main>
  );
